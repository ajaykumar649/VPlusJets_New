...
const TString inDataDir  = "/eos/uscms/store/user/ajay/try/";
const TString inQCDDir  = "/eos/uscms/store/user/ajay/try/";
...
void kanamuon::myana(double myflag, bool isQCD, int runflag)
{
	...
	if (myflag == 20120000 || myflag == -100){
		...
		if ( !isQCD ) {
			...
			Init(myChain);Loop( h_events, h_events_weighted, 20120000,runflag, outDataDir + "RD_WmunuJets_DataAll_GoldenJSON_v2_fb");
